package com.goode.imgcompress.reduce.util;
...
public final class Util {
    ...
    /**
     * Convert the string into MD5 format
     *
     * @param string
     */
    ...
    public static boolean isPicFile(String filePath) {
        ...
        if (suffix.equalsIgnoreCase(ImageFormat.JPEG.getType())
                || suffix.equalsIgnoreCase(ImageFormat.JPG.getType())
                || suffix.equalsIgnoreCase(ImageFormat.PNG.getType())
                || suffix.equalsIgnoreCase(ImageFormat.WEBP.getType())) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * 根据尺寸或大小获取inSampleSize
     *
     * @param srcPath         源图路径
     * @param compressOptions 压缩选项
     * @return inSampleSize
     */
    public static int getInSampleSizeByWHorLength(@NonNull String srcPath, @NonNull CompressOptions compressOptions) {
        if (TextUtils.isEmpty(srcPath)) {
            throw new IllegalArgumentException("The src path is empty.");
        }
        BitmapFactory.Options newOpts = new BitmapFactory.Options();
        newOpts.inJustDecodeBounds = true;
        BitmapFactory.decodeFile(srcPath, newOpts);
        int ww = newOpts.outWidth;
        int hh = newOpts.outHeight;
        int inSampleSize1 = calculateSampleSize(ww, hh, compressOptions.getDefaultWidthHeight());
        int inSampleSize2 = calculateSampleSize(new File(srcPath).length(), compressOptions.getMaxLength());
     ...
     * @return inSampleSize
     */
    private static int calculateSampleSize(int width, int height, int[] wh) {
        if (width < 0) {
            throw new IllegalArgumentException("the width num must be >0");
        }
        if (height < 0) {
            throw new IllegalArgumentException("the height num must be >0");
        }
        int defaultWidth = wh[0];
        int defaultHeight = wh[1];
        int inSampleSize = 1;
        // 根据图片尺寸按宽或高计算缩放比
        if (width > height && width > defaultWidth) {
            inSampleSize = width / defaultWidth;
        } else if (width < height && height > defaultHeight) {
            inSampleSize = height / defaultHeight;
        }
        inSampleSize = inSampleSize > 0 ? inSampleSize : 1;
        return inSampleSize;
    }
     ...
     * @return inSampleSize
     */
    private static int calculateSampleSize(long srcFileLength, long destFileLength) {
        if (srcFileLength < 0) {
            throw new IllegalArgumentException("the length of srcFile must be >0");
        }
        if (destFileLength < 0) {
            throw new IllegalArgumentException("the length of destFile must be >0");
        }
        int inSampleSize;
        inSampleSize = (int) (Math.ceil(Math.sqrt(srcFileLength / destFileLength)));
        inSampleSize = inSampleSize > 0 ? inSampleSize : 1;
        return inSampleSize;
    }
}
