...
VN_STATUS
vnBicubicKernel(CONST CVImage &pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC, FLOAT32 fX, FLOAT32 fY,
                UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
    ...
    for (INT32_T j = -2 + 1; j <= 2; j++)
        for (INT32_T i = -2 + 1; i <= 2; i++) {
...
VN_STATUS
vnSampleBicubicVertical(CONST CVImage &pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC, FLOAT32 fX,
                        FLOAT32 fY, UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
        }
...
VN_STATUS
vnSampleBicubicHorizontal(CONST CVImage &pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC, FLOAT32 fX,
                          FLOAT32 fY, UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
...
VN_STATUS
vnBicubicKernel(CONST CVImage &pSrcImage, FLOAT32 fCoeffB, FLOAT32 fCoeffC, FLOAT32 fX, FLOAT32 fY,
                BOOL bDirection, UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
        }
    ...
    switch (bDirection) {
        case FALSE:
            return vnSampleBicubicHorizontal(pSrcImage, fCoeffB, fCoeffC, fX, fY, pRawOutput);
        case TRUE:
            return vnSampleBicubicVertical(pSrcImage, fCoeffB, fCoeffC, fX, fY, pRawOutput);
    }
