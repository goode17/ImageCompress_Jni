...
VN_STATUS vnLanczosKernel(CONST CVImage &pSrcImage, FLOAT32 fCoeffN, FLOAT32 fX, FLOAT32 fY,
                          UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
    ...
    INT32_T iRadius = fCoeffN;

    for (INT32_T j = -iRadius + 1; j <= iRadius; j++)
        for (INT32_T i = -iRadius + 1; i <= iRadius; i++) {
            INT32_T iX = (INT32_T) fX + i;
            INT32_T iY = (INT32_T) fY + j;
...
VN_STATUS vnSampleLanczosVertical(CONST CVImage &pSrcImage, FLOAT32 fCoeffN, FLOAT32 fX, FLOAT32 fY,
                                  UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
    ...
    for (INT32_T j = -iRadius; j < iRadius; j++) {
...
VN_STATUS
vnSampleLanczosHorizontal(CONST CVImage &pSrcImage, FLOAT32 fCoeffN, FLOAT32 fX, FLOAT32 fY,
                          UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
    ...
    for (INT32_T i = -iRadius; i < iRadius; i++) {
        INT32_T iX = (INT32_T) fX + i;
...
VN_STATUS
vnLanczosKernel(CONST CVImage &pSrcImage, FLOAT32 fCoeffN, FLOAT32 fX, FLOAT32 fY, BOOL bDirection,
                UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
    ...
    switch (bDirection) {
        case FALSE:
            return vnSampleLanczosHorizontal(pSrcImage, fCoeffN, fX, fY, pRawOutput);
        case TRUE:
            return vnSampleLanczosVertical(pSrcImage, fCoeffN, fX, fY, pRawOutput);
    }
