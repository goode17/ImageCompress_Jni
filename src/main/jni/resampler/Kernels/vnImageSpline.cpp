...
VN_STATUS
vnSampleSplineVertical(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
        }
...
VN_STATUS
vnSampleSplineHorizontal(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
        }
...
VN_STATUS vnSplineKernel(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, BOOL bDirection,
                         UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (!pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
    ...
    switch (bDirection) {
        case FALSE:
            return vnSampleSplineHorizontal(pSrcImage, fX, fY, pRawOutput);
        case TRUE:
            return vnSampleSplineVertical(pSrcImage, fX, fY, pRawOutput);
    }
