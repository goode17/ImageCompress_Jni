...
VN_STATUS vnAverageKernel(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, FLOAT32 fRadius,
                          UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
...
VN_STATUS vnSampleAverageVertical(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, FLOAT32 fRadius,
                                  UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (0 == fRadius || !pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
...
VN_STATUS
vnSampleAverageHorizontal(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, FLOAT32 fRadius,
                          UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (0 == fRadius || !pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
...
VN_STATUS
vnAverageKernel(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, BOOL bDirection, FLOAT32 fRadius,
                UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (0 == fRadius || !pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
    ...
    switch (bDirection) {
        case FALSE:
            return vnSampleAverageHorizontal(pSrcImage, fX, fY, fRadius, pRawOutput);
        case TRUE:
            return vnSampleAverageVertical(pSrcImage, fX, fY, fRadius, pRawOutput);
    }
