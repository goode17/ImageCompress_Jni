...
VN_STATUS
vnSampleCoverageVertical(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, FLOAT32 fRadius,
                         UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (0 == fRadius || !pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
...
VN_STATUS
vnSampleCoverageHorizontal(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, FLOAT32 fRadius,
                           UINT8 *pRawOutput) {
    if (VN_PARAM_CHECK) {
        if (0 == fRadius || !pRawOutput || !VN_IS_IMAGE_VALID(pSrcImage)) {
            return vnPostError(VN_ERROR_INVALIDARG);
...
vnCoverageKernel(CONST CVImage &pSrcImage, FLOAT32 fX, FLOAT32 fY, BOOL bDirection, FLOAT32 fRadius,
    ...
    switch (bDirection) {
        case FALSE:
            return vnSampleCoverageHorizontal(pSrcImage, fX, fY, fRadius, pRawOutput);
        case TRUE:
            return vnSampleCoverageVertical(pSrcImage, fX, fY, fRadius, pRawOutput);
    }
