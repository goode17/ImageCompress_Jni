...
VN_STATUS
vnBresenhamScaleLine(UINT8 *pSrcBuffer, UINT32 uiSrcLength, UINT32 uiSrcStride, UINT32 uiPixelPitch,
                     UINT8 *pDestBuffer, UINT32 uiDestLength, UINT32 uiDestStride) {
    if (VN_PARAM_CHECK) {
        if (!pSrcBuffer || 0 == uiSrcLength || !pDestBuffer || 0 == uiDestLength ||
            0 == uiPixelPitch) {
...
VN_STATUS
vnScaleImage(CONST CVImage &pSrcImage, UINT32 uiWidth, UINT32 uiHeight, CVImage *pDestImage) {
    if (VN_PARAM_CHECK) {
