...
VN_STATUS
vnResizeImageNonSeparable(CONST CVImage &pSrcImage, VN_IMAGE_KERNEL_TYPE uiKernel, FLOAT32 fHRatio,
                          FLOAT32 fVRatio, CVImage *pDestImage) {
    ...
    for (UINT32 j = 0; j < pDestImage->QueryHeight(); j++)
        for (UINT32 i = 0; i < pDestImage->QueryWidth(); i++) {
            ...
            if (VN_FAILED(
                    vnSampleImage(pSrcImage, uiKernel, VN_IMAGE_KERNEL_2D_COMBINED, fX, fY, fRadius,
                                  pOutputData))) {
...
VN_STATUS
vnResizeImageSeparable(CONST CVImage &pSrcImage, VN_IMAGE_KERNEL_TYPE uiKernel, FLOAT32 fHRatio,
                       FLOAT32 fVRatio, CVImage *pDestImage) {
    CVImage tempImage;

    if (VN_FAILED(vnCreateImage(pSrcImage.QueryFormat(), pDestImage->QueryWidth(),
                                pSrcImage.QueryHeight(), &tempImage))) {
        return vnPostError(VN_ERROR_EXECUTION_FAILURE);
    }
    ...
    for (UINT32 j = 0; j < pSrcImage.QueryHeight(); j++)
        for (UINT32 i = 0; i < pDestImage->QueryWidth(); i++) {
            ...
            if (VN_FAILED(vnSampleImage(pSrcImage, uiKernel, VN_IMAGE_KERNEL_1D_HORIZONTAL, fX, fY,
                                        fHRatio, pOutputData))) {
                return vnPostError(VN_ERROR_EXECUTION_FAILURE);
            }
        }
    ...
    for (UINT32 j = 0; j < pDestImage->QueryHeight(); j++)
        for (UINT32 i = 0; i < pDestImage->QueryWidth(); i++) {
            ...
            if (VN_FAILED(
                    vnSampleImage(tempImage, uiKernel, VN_IMAGE_KERNEL_1D_VERTICAL, fX, fY, fVRatio,
                                  pOutputData))) {
                return vnPostError(VN_ERROR_EXECUTION_FAILURE);
            }
...
VN_STATUS vnResizeImage(CONST CVImage &pSrcImage,
    ...
    FLOAT32 fHorizRatio = (1 == uiWidth ? 1.0f : (FLOAT32) (pSrcImage.QueryWidth() - 1) /
                                                 (uiWidth - 1));
    FLOAT32 fVertRatio = (1 == uiHeight ? 1.0f : (FLOAT32) (pSrcImage.QueryHeight() - 1) /
                                                 (uiHeight - 1));
