...
VN_STATUS vnPackFloatChannel(CONST FLOAT64 &pInChannel, UINT8 uiChannelRate, UINT8 *pRawChannel) {
    ...
    switch (uiChannelRate) {
        case 16:
            *((FLOAT16 *) pRawChannel) = (FLOAT32) pInChannel;
            break;
        case 32:
            *((FLOAT32 *) pRawChannel) = (FLOAT32) pInChannel;
            break;
...
VN_STATUS
vnPackFixedChannel(INT64 pInChannel, BOOL bSigned, UINT8 uiChannelRate, UINT32 uiSumOffset,
                   UINT64 *pRawData) {
    //
    ...
    UINT64 uiMaskChannelVal =
            ((UINT64) 1 << uiChannelRate) - 1;     // bit mask for our destination channel
...
VN_STATUS
vnPackFloatPixel(CONST VN_PIXEL_BLOCK &pSourceBlock, VN_IMAGE_FORMAT format, UINT8 *pRawPixel) {
...
VN_STATUS
vnPackFixedPixel(CONST VN_PIXEL_BLOCK &pSourceBlock, VN_IMAGE_FORMAT format, UINT8 *pRawPixel) {
    //
    ...
    for (UINT32 i = 0; i < uiChannelCount; i++) {
        ...
        if (VN_FAILED(vnPackFixedChannel(pSourceBlock.iChannelData[i], VN_IS_SIGNED_FORMAT(format),
                                         uiChannelRate, uiSumOffset, &uiRawData))) {
            return vnPostError(VN_ERROR_EXECUTION_FAILURE);
        }
...
VN_STATUS
vnConvertFromBlock(CONST VN_PIXEL_BLOCK &pSourceBlock, VN_IMAGE_FORMAT format, UINT8 *pRawPixel) {
    if (VN_PARAM_CHECK) {
    ...
    VN_IMAGE_PRECISION formatPrecision = bFloatFormat ? VN_IMAGE_PRECISION_FLOAT
                                                      : VN_IMAGE_PRECISION_FIXED;
